...
public class GameLoop extends ApplicationAdapter {
  ...
  @Override
  public void create() {
    socketHandler.setConnectListener(
        session -> events.add(session.getId() + " just joined"));
    socketHandler.setDisconnectListener(
        session -> events.add(session.getId() + " just disconnected"));
    socketHandler.setMessageListener(
        (session, message) -> events.add(session.getId() + " said " + message));
  }
